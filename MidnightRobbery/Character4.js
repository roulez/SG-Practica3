...
class Character4 extends THREE.Object3D {
  ...
  constructor (parameters) {
    ...
    var texturaTraje = loaderTraje.load("imgs/traje.jpg");
    this.materialTraje = new THREE.MeshPhongMaterial({map: texturaTraje});
    
    var loaderTshirt = new THREE.TextureLoader();
    var texturaTshirt = loaderTshirt.load("imgs/eyes.jpg");
    this.materialTshirt = new THREE.MeshPhongMaterial({map: texturaTshirt});
    
    var loaderPantalones = new THREE.TextureLoader();
    var texturaPantalones = loaderPantalones.load("imgs/pantalonesTraje.jpeg");
    this.materialPantalones = new THREE.MeshPhongMaterial({map: texturaPantalones});
  ...
  createRightLeg(anguloX, anguloZ){
    ...
    var legMesh = new THREE.Mesh(legGeo, this.materialPantalones);

    //Pie        
    var pieGeo = new THREE.SphereGeometry(1, 20, 50);
    pieGeo.applyMatrix(new THREE.Matrix4().makeTranslation(0,-10,0));   
    var pieMesh = new THREE.Mesh(pieGeo, this.materialPantalones);
  ...
  createHair(){
    ...
    var texture = new THREE.Texture();
    var loader = new THREE.ImageLoader(manager);
    loader.load("imgs/grey.jpg", function(image) {
        texture.image = image;
        texture.needsUpdate = true;
    });
    ...
    ObjLoader1.load('hairRear.obj', function (object) {
        object.traverse(function (child) {
            if(child instanceof THREE.Mesh)
                child.material.map = texture;
        });
    ...
    ObjLoader2.setPath('recursos/hair/');
    ObjLoader2.load('hairFront.obj', function (object) {
        object.traverse(function (child) {
            if(child instanceof THREE.Mesh)
                child.material.map = texture;
        });
